const API_BASE = 'https://script.google.com/macros/s/AKfycbx6KkxCKeA6JClmdQScS3fgVuVG0YsuGyhyJHB3An0JbgPqLViXUCmL6_sUZfYRxONG6g/exec';
...
const FIELD_HELP = {
  categoria: 'Agrupa o item por tipo, como Escritório, EPI, Higienização ou Limpeza.',
  unidade: 'Unidade de medida usada no controle do estoque, como un, cx, pct, resma.',
  ...
  controla_lote: 'Use Sim quando o item precisa controle por lote.',
  controla_validade: 'Use Sim quando o item possui validade e ela precisa ser acompanhada.'
};
...
function helpChip(label, text) {
  return `<div class="help-chip"><strong>${label}:</strong> ${esc(text)}</div>`;
}
...
function renderDashboard() {
  ...
  q('dashboardView').innerHTML = `
    <div class="cards">
      <div class="kpi"><div class="label">Total de itens</div><div class="value">${fmt(d.totalItens)}</div></div>
      <div class="kpi"><div class="label">Estoque baixo</div><div class="value">${fmt(d.estoqueBaixo)}</div></div>
      <div class="kpi"><div class="label">Validades próximas</div><div class="value">${fmt(d.validades)}</div></div>
      <div class="kpi"><div class="label">Saldo total</div><div class="value">${fmt(d.saldoTotal)}</div></div>
    </div>
  ...
  q('itemsView').innerHTML = `
    <div class="panel compact-panel">
      ...
      <div class="info-grid">
        ...
        ${helpChip('Lote/Validade', 'Os campos Sim/Não indicam se o item precisa controlar lote de fabricação e validade.')}
      </div>
    ...
    <div class="panel" style="margin-top:16px">
      ...
      <div class="filters">
        <input placeholder="Buscar por código ou item" value="${esc(state.filters.item)}"
...
async function saveMovement() {
  ...
  if (r.ok) {
    ['m_codigo', 'm_item', 'm_quantidade', 'm_unidade', 'm_responsavel', 'm_lote', 'm_validade', 'm_doc', 'm_fornecedor', 'm_obs']
...
function renderMovs() {
  ...
  q('movsView').innerHTML = `
    <div class="panel">
      ...
      <div class="form-grid compact-form-grid">
        <div><label>Código</label><input id="m_codigo" placeholder="Código do item" /></div>
        <div><label>Item</label><input id="m_item" placeholder="Nome do item" /></div>
        <div><label>Tipo</label><select id="m_tipo"><option>Entrada</option><option>Saída</option></select></div>
        <div><label>Quantidade</label><input id="m_quantidade" type="number" placeholder="Quantidade" /></div>
        <div><label>Unidade destino/origem</label><input id="m_unidade" list="dl_mov_unidades" placeholder="Unidade" /></div>
        <div><label>Responsável</label><input id="m_responsavel" placeholder="Responsável" /></div>
        <div><label>Lote</label><input id="m_lote" placeholder="Lote" /></div>
        <div><label>Validade</label><input id="m_validade" type="date" /></div>
        <div><label>Documento</label><input id="m_doc" placeholder="NF, requisição..." /></div>
        <div><label>Fornecedor</label><input id="m_fornecedor" list="dl_mov_fornecedores" placeholder="Fornecedor" /></div>
        <div><label>Observações</label><input id="m_obs" placeholder="Observações" /></div>
      </div>
    ...
    <div class="panel" style="margin-top:16px">
      ...
      <div class="filters">
        <input placeholder="Buscar por item ou código" value="${esc(state.filters.mov)}"
  ...
  q('unitsView').innerHTML = `
    <div class="panel">
      ...
      <div class="table-wrap">
        <table>
          <thead>
            <tr><th>Código</th><th>Unidade</th><th>Tipo</th><th>Região</th><th>Responsável</th></tr>
          </thead>
          <tbody>
            ${state.units.map(r => `
              <tr>
  ...
  q('suppliersView').innerHTML = `
    <div class="panel">
      ...
      <div class="table-wrap">
        <table>
          <thead>
            <tr><th>Fornecedor</th><th>CNPJ</th><th>Cidade</th><th>UF</th></tr>
          </thead>
          <tbody>
...
function bindEvents() {
  ...
  q('sidebarOverlay')?.addEventListener('click', () => {
    closeSidebarMobile();
  });
