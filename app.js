  ...
  q('movsView').innerHTML = `
    <div class="panel">
      ...
      <div class="info-grid">
        ...
        ${helpChip('Item', 'Selecione o código do item cadastrado; o nome será preenchido automaticamente.')}
      </div>

      <div class="form-grid compact-form-grid">
        <div>
          abel>Item cadastrado</label>
          <select id="m_item_codigo" onchange="onMovementItemChange()">
            <option value="">Selecione um item...</option>
            ${itensOrdenados.map(i => `
              <option value="${esc(i.codigo)}">${esc(i.codigo)} - ${esc(i.item)}</option>
            `).join('')}
          </select>
        </div>
        <div>
          abel>Nome do item</label>
